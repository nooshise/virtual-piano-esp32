...
|---|---|---|---|
| `TFT9341Touch` / קובץ מקומי | local header | UI | נמצא |
| `Adafruit_GFX` | 1.12.6 | UI | נמצא |
| `Adafruit_ILI9341` | 1.6.3 | UI | נמצא |
| `Adafruit_STMPE610` | 1.1.6 | UI | נמצא |
...
|---|---|---:|---|---|
| UI | לא בוצעה העלאה | ___ | לא הורץ | קומפילציה בלבד |
| Sensor | לא בוצעה העלאה | ___ | לא הורץ | לא נדרש במשימה זו |
...
|---|---:|---|---|---|
| UI | 115200 | `[TFT] begin`, `[TOUCH] OK` או `[TOUCH] FAIL`, `[SCREEN] name=MAIN` | לא נבדק על חומרה | לא הורץ |
| Sensor | 115200 | __________ | לא נבדק על חומרה | לא הורץ |
...
בוצעה קומפילציה ל-`instrument_config` בלבד לאחר שינויי TFT/touch.
